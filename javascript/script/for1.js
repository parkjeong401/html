...
const kiosk = ['아메리카노','카페라떼','카푸치노','돌체라떼','에스프레소','우유','녹차라떼','소이라뗴','밀크티']
...
const checkSelect = document.querySelectorAll('.select input');
...
gnb[0].addEventListener('mouseover',()=>sub[1].style.display = 'block')
gnb[0].addEventListener('mouseover',()=>sub[2].style.display = 'block')
gnb[0].addEventListener('mouseover',()=>sub[3].style.display = 'block') */

for(let i of sub) console.log(i);
for(let i of gnb){
