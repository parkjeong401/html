...
//getDate()
// getFullYear()
// 위 날짜 메서드를 실행하기 위한 초기 세팅으로 new Date() 메소르 반드시선언!
// 자바스크립트는 숫자를 0부터 처리한다. 그래서 getMonth로 월 출력 시 +1을 반드시 작성함
let now = new Date();
...
//메서드와 속성을 작성할 때는 반드시 앞에 객체(object)선언이 되어야 함
// 객체.메서드();
// 객체.속성;
// =========================배열과 객체
// 배열 이전 형태 기본 변수
...
console.log(`box1 의 값은 ${box1}이다.`);
// 배열 기본 문법과 활용
let yoil = ['일','월','화','수','목','금','토'];
console.log(yoil);
//배열의 개별 데이터 인덱스는 0부터 인식한다.
//배열의 개별 인덱스 호출하는 방법(개별 호출안하면 무조건 전체출력만 됨)
...
//배열 생성과 호출 연습
let colorSet = ['skyblue','red','gray','black','brown'];
console.log(colorSet[0],colorSet[2],colorSet[1]);//랜덤으로 3개 고르기
let fruits = ['귤','사과','오렌지','바나나','딸기','복숭아','수박'];
//주문하신 상품은 복숭아,오렌지,수박,귤,사과,딸기, 바나나 총 7개 입니다.
console.log(`주문하신 상품은 ${fruits[6]}, ${fruits[2]}, ${fruits[3]}, ${fruits[0]}, ${fruits[1]}, ${fruits[5]}, ${fruits[4]} 총 ${fruits.length}개 입니다.`);
//배열 추가 메서드 공부
...
numbers.push(10);
//메가박스 빠른 예매 시스템 이해
// 1. 인원 선택 안할 시 좌석은 빈 값으로 빈 배열로 시작함
// 2. 인원 추가에 따라 배열에 좌석값이 들어갈 수 있는 저장소가 추가됨
// 3. 추가된 인원이 희망좌석을 선택하면 해당 좌석정보가 위 2번 저장소에 대입됨
let megabox = [];
...
let array2 = new Array(5,6,7,8); //새로운 배열생성 뜻으로 작성하는 문법
let array3 = new Array(4); //값 4가 아닌 배열 4자리를 준비한다는 뜻
console.log(array1);
console.log(array2);
console.log(array3);
// 배열 주의사항,호출 시 반드시 인덱스를 사용함. array1(x) array1[0] (O)