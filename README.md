...
* 부모-자식 구조 항상 함꼐 묶여있어야 하는 태그 작성시에는 >를 사용함
원하는 태그 꺽쇠없이 단어만 작성하고 단축키 tab
* 필수속성이 존재한다면 함께 나타남(일부표시 안될 수 잇으니 반드시 체크!)
* 특정태그를 한번에 여러개 만들고 싶다면 "*" 이용함
* 2개 이상의 형제 태그를 만들고 싶다면 "+" 이용함
* class,id적용한 태그를 만들고 싶다면 "#, ."이용함 